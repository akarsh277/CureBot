...
function safeWSSend(obj) {
  try {
    if (!ws || ws.readyState !== WebSocket.OPEN) {
      initWebSocket();
      addMessage("Connecting to backend, try again in a moment.", "bot");
      return false;
    }
...
const Q = {
  ...
  askGender: {
    english: "What is your gender? (Male / Female)",
    telugu: "మీ లింగం ఏమిటి? (Male / Female)",
    hindi: "आपका लिंग क्या है? (Male / Female)"
  },
  askSymptoms: {
...
window.onload = () => {
  ...
  // ask language with voice
  setTimeout(() => {
    askLanguage(false); // autoListen = false to prevent the microphone from picking up the bot's own voice
  }, 900);
};
